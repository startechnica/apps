...
{{- $enabled := list -}}
{{- if .Values.modules.eap.tls.enabled -}}{{- $enabled = append $enabled "tls" -}}{{- end -}}
{{- if .Values.modules.eap.ttls.enabled -}}{{- $enabled = append $enabled "ttls" -}}{{- end -}}
{{- if eq (len $enabled) 0 -}}
freeradius: modules.eap
    `modules.eap.enabled: true` but no EAP method is enabled. Enable at least
    one of `modules.eap.tls.enabled` / `modules.eap.ttls.enabled`.
{{- else if not (has $type $enabled) -}}
freeradius: modules.eap.defaultType
    `modules.eap.defaultType: {{ $type }}` must be one of the enabled methods
    ({{ join ", " $enabled }}). Either change `defaultType` or enable that
    method (`modules.eap.<method>.enabled`).
{{- else if and (not .Values.modules.eap.tlsConfig.autoGenerated) (not .Values.modules.eap.tlsConfig.certificatesSecret) -}}
freeradius: modules.eap.enabled
    ...
    the TLS-based EAP methods cannot start without one. Set one of:
      ...
      - modules.eap.tlsConfig.certificatesSecret: <name> (BYO Secret already in the namespace)
{{- end -}}
{{- end -}}
{{- end -}}
