...
- **EAP module support (rlm_eap) — EAP-TLS + EAP-TTLS.** New `modules.eap:`
  values block and `files/modules/eap`, wired into the modules ConfigMap
  when `modules.eap.enabled: true`. The values mirror FreeRADIUS's own
  structure:
  - `modules.eap.tlsConfig.*` — the shared `tls-config tls-common` block
    (server cert + TLS engine settings:
    `{autoGenerated,certificatesSecret,cert*Filename,privateKeyPassword,
    randomFile,cipherList,cipherServerPreference,minVersion,maxVersion,
    cache.enabled,ocsp.enabled}`). A fourth client-side TLS context, mounted
    at `/opt/startechnica/freeradius/certs-eap`, signed off the chart's
    shared internal CA via `templates/secret/eap-tls.yaml` — mandatory
    whenever the module is enabled.
  - `modules.eap.{tls,ttls,mschapv2,md5}` — the individual EAP method
    blocks (flat under `eap`, not nested under a `methods` key), each
    rendered (or omitted) by an `enabled` flag. `tls` / `ttls` are the
  ...
  auto-generated into the credentials Secret. The `freeradius.validate.eap`
  aggregator rejects no-method-enabled, a `defaultType` that isn't an enabled
  method, and `enabled: true` with no `tlsConfig` cert source. No `dh_file`
  is referenced (FreeRADIUS 3.2 negotiates ephemeral DH/ECDHE; the chart
  ships no DH params file). Unlike the other four module files (vendored,
  loaded verbatim via `AsConfig`), `files/modules/eap` is rendered through
  Helm `tpl` so structural values are inlined directly from `.Values` and
  method blocks are conditionally emitted — only the private-key password
